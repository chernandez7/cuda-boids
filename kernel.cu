...
void normalizeVector(float3 vector) {
	...
	if (magnitude > 0) {
		divVectorByScalar(magnitude, vector);
	}
}
...
__device__
float3 RNG(unsigned int seed, int index, int max) {
	curandState_t state;
	curand_init(seed + index, 0, 0, &state);
	return make_float3(
		(curand(&state) % max) - max,
		(curand(&state) % max) - max,
		(curand(&state) % max) - max
	);
}
...
__global__
void generateRandomPosArray(int n, unsigned int seed, float4* arr, float mass) {
	int index = (blockIdx.x * blockDim.x) + threadIdx.x;
	
	if (index < n) {
		float3 random = RNG(seed, index, 500);
...
__global__
void generateRandomArray(int n, unsigned int seed, float3* arr) {
	int index = (blockIdx.x * blockDim.x) + threadIdx.x;

	if (index < n) {
		float3 random = RNG(seed, index, 10);
...
float3 SeparationRule(int n, float4* pos, float3* vel, float3 target) {
	...
	if (index < n) {
		...
		for (int i = 0; i < n; i++) {
			...
			float distanceToNeighbor = distanceFormula(myPosition, target);//their
...
float3 AlignmentRule(int n, float4* pos, float3* vel) {
	...
	if (index < n) {
		...
		if (numberOfNeighbors > 0) {
			...
			return limit(5, sub2VectorsNew(sum, myVelocity));
		}
		else {
			// No neighbors nearby
			return make_float3(0, 0, 0);
...
void initCuda(int n) {
	...
	generateRandomPosArray << <fullBlocksPerGrid, BlockSize >> >(n, time(NULL), dev_pos, boidMass);
	generateRandomArray << <fullBlocksPerGrid, BlockSize >> >(n, time(NULL), dev_vel);
	generateRandomArray << <fullBlocksPerGrid, BlockSize >> >(n, time(NULL), dev_acc);
}
